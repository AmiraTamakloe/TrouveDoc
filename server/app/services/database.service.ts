...
export class DatabaseService {
  ...
  public async addMedecin(medecin: any): Promise<pg.QueryResult> {
    ...
    const query: string = `INSERT INTO Medecins ( prenom, nom, specialite, anneesExperience, idService) VALUES ($1, $2, $3, $4, $5)`;
  ...
  public async deleteMedecin(idMedecin: string): Promise<pg.QueryResult> {
    const client: pg.PoolClient = await this.pool.connect();
    const query: string = `DELETE FROM Medecins WHERE idMedecin = '${idMedecin}'`;
  ...
  public async updateMedecinInfo(medecin: any): Promise<pg.QueryResult> {
    const client: pg.PoolClient = await this.pool.connect();
    const query: string = `UPDATE Medecins SET prenom = '${medecin.prenom}', nom = '${medecin.nom}', specialite = '${medecin.specialite}', anneesExperience = ${medecin.anneesexperience}, idService = ${medecin.idservice} WHERE idMedecin = ${medecin.idmedecin}`;
    console.log(query);
    const result: pg.QueryResult = await this.pool.query(query);
    console.log(result);
    client.release();
